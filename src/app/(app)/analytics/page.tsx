...
import { AlertTriangle, BarChart2, Brain, Calendar, ChevronLeft, ChevronRight, Clock, Download, Info, TrendingDown, TrendingUp } from "lucide-react";
...
const heatmapHours = Array.from({ length: 24 }, (_, i) => ({
  ...
  value: i >= 8 && i <= 17 ? Math.floor(Math.random() * 3) + 2 : i >= 22 || i <= 5 ? Math.floor(Math.random() * 5) : Math.floor(Math.random() * 2),
}));
...
function AnalyticsContent() {
  ...
  useEffect(() => {
    if (activeTabParam && ["overview", "behavior", "risk", "timeline", "weekly"].includes(activeTabParam)) {
  ...
  return (
    <div className="space-y-6 font-poppins">
      ...
      <div>
        ...
        <p className="text-sm text-muted font-light mt-1">Laporan terperinci mengenai durasi, kebiasaan, tingkat risiko, dan timeline screen time Anda</p>
      </div>
        ...
        <div className="space-y-6 animate-page-enter">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-card border border-border rounded-3xl p-5 md:p-6 shadow-xs">
              <h4 className="font-bold text-sm text-primary mb-4">Total Screen Time per Aplikasi (Menit)</h4>
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <RechartsBarChart data={overviewScreenTime} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                    ...
                    />
                    <Bar dataKey="menit" fill="#113a5d" radius={[6, 6, 0, 0]} maxBarSize={45} />
                  </RechartsBarChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-3xl p-5 md:p-6 shadow-xs flex flex-col justify-between">
              <h4 className="font-bold text-sm text-primary mb-4">Proporsi Penggunaan</h4>
              <div className="h-44 w-full relative flex items-center justify-center">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={overviewScreenTime} cx="50%" cy="50%" innerRadius={50} outerRadius={70} paddingAngle={3} dataKey="menit">
            ...
            <div className="lg:col-span-2 bg-card border border-border rounded-3xl p-5 md:p-6 shadow-xs">
              <h4 className="font-bold text-sm text-primary mb-4">Trend Penggunaan Harian (Menit)</h4>
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={dailyTrend} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                    ...
                    />
                    <Line type="monotone" dataKey="menit" stroke="#062743" strokeWidth={3} activeDot={{ r: 6 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-3xl p-5 md:p-6 shadow-xs">
              <h4 className="font-bold text-sm text-primary mb-2">Jam Aktif Terpadat</h4>
      ...
      {activeTab === "behavior" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-page-enter">
          ...
          <div className="bg-card border border-border rounded-3xl p-6 shadow-xs flex flex-col items-center justify-between text-center min-h-[340px]">
            <div>
              <h3 className="font-bold text-base text-primary">Skor Perilaku Digital</h3>
              <p className="text-[10px] text-muted font-light mt-0.5">Penilaian berdasarkan analisis kebiasaan</p>
            </div>
            <div className="my-6 relative flex items-center justify-center">
              ...
              <div className="w-36 h-36 rounded-full border-12 border-muted-light flex items-center justify-center relative">
                ...
                <div className="flex flex-col items-center justify-center z-10">
                  ...
                  <span className="text-xs text-muted font-semibold">Cukup Baik</span>
                </div>
              </div>
            </div>
            <div className="space-y-1">
              <span className="inline-block text-[10px] font-extrabold px-3 py-1 rounded-full bg-amber-100 text-amber-800 uppercase tracking-wide">Batas Wajar</span>
              <p className="text-[10px] text-muted leading-tight font-light mt-1">Sedikit di atas standar sehat. Disarankan mengurangi frekuensi cek instan.</p>
            </div>
          </div>

          {/* Breakdown skor per indikator */}
          <div className="bg-card border border-border rounded-3xl p-6 shadow-xs lg:col-span-2">
            <h3 className="font-bold text-base text-primary mb-4">Breakdown Indikator Skor</h3>
          ...
          <div className="bg-card border border-border rounded-3xl p-6 shadow-xs">
            <h3 className="font-bold text-base text-primary mb-4">Radar Karakteristik</h3>
            <div className="h-64 w-full flex items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart cx="50%" cy="50%" outerRadius="75%" data={behaviorRadarData}>
                  <PolarGrid />
          ...
          <div className="bg-card border border-border rounded-3xl p-6 shadow-xs lg:col-span-2">
            <h3 className="font-bold text-base text-primary mb-4">Trend Skor Perilaku (Mingguan)</h3>
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
      ...
      {activeTab === "risk" && (
        <div className="space-y-6 animate-page-enter">
          ...
          <div className="bg-card border border-border rounded-3xl p-6 md:p-8 shadow-xs">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="space-y-3 max-w-xl">
                <span className="inline-block text-[10px] font-extrabold px-3 py-1 rounded-full bg-amber-100 text-amber-800 uppercase tracking-wide">Analisis Risiko</span>
                <h2 className="text-2xl font-black text-primary">Tingkat Risiko: MODERATE (Sedang)</h2>
                <p className="text-sm text-muted font-light leading-relaxed">
                  Tingkat risiko Anda saat ini tergolong sedang. Ini berarti Anda memiliki kontrol diri yang cukup baik pada siang hari, namun rentan terpengaruh distraksi impulsif di jam malam atau
              ...
              <div className="w-32 h-32 rounded-3xl bg-amber-50 border border-amber-200 flex flex-col items-center justify-center text-center shrink-0 self-center">
                ...
                <span className="text-xl font-black text-amber-800">MODERATE</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-card border border-border rounded-3xl p-6 shadow-xs">
              <h3 className="font-bold text-base text-primary mb-4">Kontributor Risiko Tertinggi</h3>
              <div className="space-y-4">
                <div className="flex items-start gap-4 p-4 rounded-2xl bg-red-50/40 border border-red-100">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-primary">Midnight Usage (Penggunaan Tengah Malam)</h4>
                    <p className="text-xs text-muted font-light leading-relaxed mt-1">
                      Aktivitas screen time di atas jam 22.00 menyumbang 40% dari total kenaikan indeks risiko Anda. Menatap layar biru merusak siklus tidur alami.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4 p-4 rounded-2xl bg-amber-50/40 border border-amber-100">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-primary">Distraction Tendency (Kecenderungan Distraksi)</h4>
                    <p className="text-xs text-muted font-light leading-relaxed mt-1">Membuka aplikasi sosial media pada jam produktif (08:00 - 17:00). Hal ini mengganggu retensi fokus kerja Anda.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-3xl p-6 shadow-xs">
              <h3 className="font-bold text-base text-primary mb-6">Riwayat Tingkat Risiko Mingguan</h3>
              <div className="relative pl-6 border-l border-border space-y-6">
                {[
                  ...
                  <div key={item.week} className="relative">
                    <span className={`absolute -left-8.5 top-1.5 w-5 h-5 rounded-full border-4 ${item.status}`} />
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-semibold text-primary">{item.week}</span>
                      <span className={`text-[10px] font-extrabold px-2.5 py-0.5 rounded-full uppercase ${item.status}`}>{item.risk}</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {activeTab === "timeline" && (
        <div className="space-y-6 animate-page-enter">
          {/* Date Selector */}
          <div className="flex items-center justify-between gap-4">
            <h3 className="text-base font-bold text-primary font-poppins">Timeline Aktivitas</h3>
            <input
              type="date"
              value={selectedDate}
          ...
          <div className="bg-card border border-border rounded-3xl p-6 shadow-xs space-y-6">
            <div className="relative pl-6 border-l-2 border-border space-y-6">
              ...
              <div className="relative">
                <span className="absolute -left-9.5 top-1 w-6 h-6 rounded-full bg-red-100 border-2 border-red-500 flex items-center justify-center text-[10px] font-bold text-red-600">!</span>
                <div className="space-y-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-xs font-bold text-primary font-poppins">23:45</span>
                    <span className="text-xs font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-md border border-red-100 flex items-center gap-1 font-poppins">🔴 Midnight Usage</span>
                  </div>
                  <p className="text-sm font-semibold text-primary">Membuka Instagram selama 30 menit</p>
                  <p className="text-xs text-muted font-light">Sesi scroll impulsif sebelum tidur terdeteksi.</p>
                </div>
              </div>
              ...
              <div className="relative">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-primary">19:00</span>
                  </div>
                  <p className="text-sm font-semibold text-primary">Membuka TikTok selama 40 menit</p>
                  <p className="text-xs text-muted font-light">Sesi santai setelah makan malam.</p>
                </div>
              </div>

              {/* Item 3 */}
              <div className="relative">
                <span className="absolute -left-9.5 top-1 w-6 h-6 rounded-full bg-amber-100 border-2 border-amber-500 flex items-center justify-center text-[10px] font-bold text-amber-600">!</span>
                <div className="space-y-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-xs font-bold text-primary">15:00</span>
                    <span className="text-xs font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-md border border-amber-100 flex items-center gap-1">🟡 Productive Hour Usage</span>
                  </div>
                  <p className="text-sm font-semibold text-primary">Membuka TikTok selama 15 menit</p>
                  <p className="text-xs text-muted font-light">Membuka media sosial di tengah jam kerja/belajar produktif.</p>
                </div>
              </div>
              ...
              <div className="relative">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-primary">12:30</span>
                  </div>
                  <p className="text-sm font-semibold text-primary">Membuka YouTube selama 45 menit</p>
                  <p className="text-xs text-muted font-light">Menonton video pada jam istirahat siang.</p>
                </div>
              </div>
              ...
              <div className="relative">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-semibold text-primary">Membuka Instagram selama 20 menit</p>
                  <p className="text-xs text-muted font-light">Sesi buka HP spontan pada jam kerja utama.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6 animate-page-enter">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-card border border-border rounded-3xl p-5 shadow-xs">
              ...
              <h3 className="text-2xl font-black text-primary mt-1">26.5 jam</h3>
            ...
            <div className="bg-card border border-border rounded-3xl p-5 shadow-xs">
              <span className="text-[10px] font-bold text-muted uppercase tracking-wider block">Rata-rata Skor</span>
            ...
            <div className="bg-card border border-border rounded-3xl p-5 shadow-xs">
              <span className="text-[10px] font-bold text-muted uppercase tracking-wider block">Rata-rata Risiko</span>
              <h3 className="text-2xl font-black text-amber-700 mt-1">Moderate</h3>
              <span className="text-[9px] text-muted font-light block mt-1.5">Sama dengan minggu lalu</span>
            </div>

            <div className="bg-card border border-border rounded-3xl p-5 shadow-xs">
              <span className="text-[10px] font-bold text-muted uppercase tracking-wider block">Kategori Risiko</span>
              <h3 className="text-2xl font-black text-emerald-700 mt-1">Stabil</h3>
              <span className="text-[9px] text-emerald-600 font-semibold block mt-1.5">Kondisi terkontrol</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-red-50/20 border border-red-100 rounded-3xl p-5">
                  <span className="text-[10px] font-bold text-red-800 uppercase tracking-wider block">Worst Day (Hari Terburuk)</span>
                  <h4 className="text-lg font-black text-primary mt-1">Kamis, 21 Mei</h4>
                  <p className="text-xs text-muted font-light mt-1">5.2 jam total penggunaan, dengan Midnight Usage aktif pukul 23:30.</p>
                </div>
                <div className="bg-emerald-50/20 border border-emerald-100 rounded-3xl p-5">
                  <span className="text-[10px] font-bold text-emerald-800 uppercase tracking-wider block">Best Day (Hari Terbaik)</span>
                  <h4 className="text-lg font-black text-primary mt-1">Selasa, 19 Mei</h4>
                  <p className="text-xs text-muted font-light mt-1">2.1 jam total penggunaan. Produktivitas jam kerja tercapai 92%.</p>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-tr from-card to-muted-light/20 border border-border rounded-3xl p-6 space-y-3">
                ...
                <p className="text-xs text-muted font-poppins font-light leading-relaxed">
                  Pola screen time Anda minggu ini menunjukkan kestabilan yang baik. Walaupun total durasi screen time menurun 8.5%, Anda masih memiliki lonjakan kebiasaan negatif di hari Kamis malam
                  (Midnight Usage). Pemicunya diduga karena kebiasaan membuka aplikasi TikTok sebelum tidur. Kami merekomendasikan untuk menaruh gadget Anda di meja kerja 30 menit sebelum jadwal tidur
            ...
            <div className="bg-card border border-border rounded-3xl p-6 shadow-xs flex flex-col justify-between">
              <div>
                <h3 className="font-bold text-sm text-primary mb-4">Top 3 Aplikasi Terbanyak</h3>
                <div className="space-y-4">
                  {[
                    {
                      rank: 1,
