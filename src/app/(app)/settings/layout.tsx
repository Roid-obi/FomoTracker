...
import { AppWindow, Bell, Lock, LogOut, User } from "lucide-react";
import Link from "next/link";
import { usePathname, useRouter } from "next/navigation";

const settingsTabs = [
  { name: "Profil Saya", href: "/settings/profile", icon: User },
  { name: "Pemantauan", href: "/settings/monitoring", icon: AppWindow },
          ...
          <div className="flex-1 space-y-3">
            <div>
              ...
              <p className="mt-1 text-xs leading-relaxed text-red-700/80">
                Logout dipindahkan ke halaman pengaturan agar Bottom Navigation
                Bar tetap bersih di mobile.
              </p>
            </div>
            <button
              type="button"
              onClick={handleLogout}
              className="inline-flex items-center gap-2 rounded-xl bg-red-600 px-4 py-2.5 text-xs font-semibold text-white transition-all hover:bg-red-700"
            >
