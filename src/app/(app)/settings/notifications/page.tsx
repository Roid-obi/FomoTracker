...
export default function NotificationsSettingsPage() {
  ...
  const [thresholdDaily, setThresholdDaily] = useState(90); // 90 minutes
  const [thresholdContinuous, setThresholdContinuous] = useState(30); // 30 minutes
  ...
  return (
    <form
      onSubmit={handleSave}
      className="grid grid-cols-1 lg:grid-cols-3 gap-6 font-poppins"
    >
      {/* Left 2 Cols: Toggles */}
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between p-4 rounded-2xl border border-border bg-card">
            <div>
              ...
              <p className="text-[10px] sm:text-xs text-muted font-light leading-relaxed">
                Peringatan dinamis saat Anda mendekati batas waktu harian yang
                Anda tetapkan.
              </p>
            </div>
            <button
              ...
              className={`w-10 h-5.5 rounded-full transition-all relative shrink-0 ${
                toggles.usageWarning ? "bg-primary" : "bg-border"
              }`}
            >
              <div
                className={`w-4 h-4 rounded-full bg-white absolute top-0.75 transition-all shadow-sm ${
                  toggles.usageWarning ? "right-0.75" : "left-0.75"
                }`}
              />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between p-4 rounded-2xl border border-border bg-card">
            <div>
              ...
              <p className="text-[10px] sm:text-xs text-muted font-light leading-relaxed">
                Pengingat otomatis untuk kembali berfokus saat membuka media
                sosial di jam produktif.
              </p>
            </div>
            <button
              type="button"
              ...
              className={`w-10 h-5.5 rounded-full transition-all relative shrink-0 ${
                toggles.focusReminder ? "bg-primary" : "bg-border"
              }`}
            >
              <div
                className={`w-4 h-4 rounded-full bg-white absolute top-0.75 transition-all shadow-sm ${
                  toggles.focusReminder ? "right-0.75" : "left-0.75"
                }`}
              />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between p-4 rounded-2xl border border-border bg-card">
            <div>
              ...
              <p className="text-[10px] sm:text-xs text-muted font-light leading-relaxed">
                Peringatan tegas saat screen time terdeteksi aktif pada jam
            ...
            <button
              ...
              className={`w-10 h-5.5 rounded-full transition-all relative shrink-0 ${
                toggles.midnightAlert ? "bg-primary" : "bg-border"
              }`}
            >
              <div
                className={`w-4 h-4 rounded-full bg-white absolute top-0.75 transition-all shadow-sm ${
                  toggles.midnightAlert ? "right-0.75" : "left-0.75"
                }`}
              />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between p-4 rounded-2xl border border-border bg-card">
            <div>
              ...
              <p className="text-[10px] sm:text-xs text-muted font-light leading-relaxed">
                Peringatan berkala saat Anda beraktivitas menatap layar tanpa
                jeda selama durasi tertentu.
              </p>
            </div>
            <button
              type="button"
              ...
              className={`w-10 h-5.5 rounded-full transition-all relative shrink-0 ${
                toggles.continuousUsage ? "bg-primary" : "bg-border"
              }`}
            >
              <div
                className={`w-4 h-4 rounded-full bg-white absolute top-0.75 transition-all shadow-sm ${
                  toggles.continuousUsage ? "right-0.75" : "left-0.75"
                }`}
              />
            </button>
          </div>
      ...
      <div className="flex flex-col gap-6">
        <div className="bg-card border border-border rounded-3xl p-6 shadow-xs space-y-4">
          ...
          <div className="space-y-4">
            <div>
              ...
              />
              <span className="text-[9px] text-muted font-light mt-1 block">
                Default: 90 menit (1.5 jam) per hari
              </span>
            </div>

            <div>
              <label
                htmlFor="threshold-continuous"
                className="block text-[10px] font-semibold text-muted mb-1.5"
              >
              ...
              />
              <span className="text-[9px] text-muted font-light mt-1 block">
                Default: 30 menit tanpa jeda istirahat
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-3xl p-5 shadow-xs flex flex-col gap-3">
          ...
          <button
            type="submit"
            className="w-full py-3 rounded-xl bg-primary text-white text-xs font-semibold hover:bg-secondary transition-all cursor-pointer shadow-xs"
          >
            Simpan Konfigurasi
          </button>
        </div>
