...
export interface UsageEventsPluginType {
  queryEvents(options: { startTime: number; endTime: number }): Promise<{ events: UsageEventData[] }>;
}

const UsageEvents = registerPlugin<UsageEventsPluginType>("UsageEvents");

export async function fetchUsageEvents(startTime: number, endTime: number): Promise<UsageEventData[]> {
  try {
 ...
 */
function calculateOverlap(sessionStart: number, sessionEnd: number, boundStart: number, boundEnd: number): number {
  const start = Math.max(sessionStart, boundStart);
  const end = Math.min(sessionEnd, boundEnd);
  return Math.max(0, end - start);
}

export function analyzeUsageEvents(events: UsageEventData[], midnightStartStr = "00:00:00", midnightEndStr = "05:00:00", productiveStartStr = "09:00:00", productiveEndStr = "17:00:00") {
  ...
  for (const event of events) {
    ...
    if (event.eventType === 1) {
      ...
      const resumedSecs = resumedDate.getHours() * 3600 + resumedDate.getMinutes() * 60 + resumedDate.getSeconds();
      const pausedSecs = pausedDate.getHours() * 3600 + pausedDate.getMinutes() * 60 + pausedDate.getSeconds();
      ...
      if (pausedSecs < resumedSecs) {
        ...
        session.midnightDurationSeconds += calculateOverlap(resumedSecs, 86400, midnightStart, midnightEnd);
        session.productiveHourDurationSeconds += calculateOverlap(resumedSecs, 86400, productiveStart, productiveEnd);

        session.midnightDurationSeconds += calculateOverlap(0, pausedSecs, midnightStart, midnightEnd);
        session.productiveHourDurationSeconds += calculateOverlap(0, pausedSecs, productiveStart, productiveEnd);
      } else {
        session.midnightDurationSeconds += calculateOverlap(resumedSecs, pausedSecs, midnightStart, midnightEnd);
        session.productiveHourDurationSeconds += calculateOverlap(resumedSecs, pausedSecs, productiveStart, productiveEnd);
      }
